...
namespace GDO.Apps.Hercules
{
    ...
    public class HerculesApp : IAppInstance
    {
        ...
        public void newClient(string cid, BrowserInfo b)
        {
            lock (_locker)
            {
                ...
                if (browserList.Count == Section.NumNodes)
                {
                    ...
                    if (controllerId != "")
                    {
                        HerculesAppHub.self.updateController(controllerId, new ControllerMessage(ConfigurationId, 1, Section.NumNodes).toString());
                    }
                }
            }
        }
        ...
        private void broadcastConfiguration()
        {
            ...
            HerculesAppHub.self.broadcastConfiguration(browserInfoJson, ConfigurationId, Id);
        }
        ...
        public void synchronize(string cid)
        {
            ...
            {
                HerculesAppHub.self.broadcastSynchronize(Id);
                syncList.Clear();
            }
        }
    ...
    public class Data
    {
        ...
        public Data(string confName)
        {
            try
            {
                using (StreamReader sr = new StreamReader("Configurations/Hercules/Data/File" + confName + ".txt"))
                {

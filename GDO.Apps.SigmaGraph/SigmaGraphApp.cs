...
namespace GDO.Apps.SigmaGraph
{
    ...
    public class SigmaGraphApp : IBaseAppInstance {
        ...
        public string ProcessGraph(string filename, bool zoomed, string folderName, int sectionWidth, int sectionHeight)
        {
            ...
            partitions = SigmaGraphPartitioning.DistributeNodesInPartitions(partitions, Nodes, Section);
            sw.Stop();
            ...
            partitions = SigmaGraphPartitioning.DistributeLinksInPartitions(partitions,Links, singleDisplayWidth, singleDisplayHeight,Section);
            sw.Stop();

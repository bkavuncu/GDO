...
namespace GDO.Apps.Maps
{
    ...
    public class MapsAppHub : Hub, IAppHub
    {
        ...
        public void AddLayer(int instanceId, string name, int sourceId, float brightness, float contrast,
            float saturation,
        ...
        public void AddHeatmapLayer(int instanceId, string name, int sourceId, float brightness, float contrast,
            float saturation,
            float hue, float opacity, int zIndex, bool visible, int minResolution, int maxResolution, string[] gradient,
            int radius, int shadow, int weight)
        {
            lock (Cave.AppLocks[instanceId])
            {
                try
                {
                    MapsApp maps = ((MapsApp) Cave.Apps["Maps"].Instances[instanceId]);
                    int layerId = -1;
                    layerId = maps.AddLayer<HeatmapLayer>(name, (int) LayerTypes.Heatmap, sourceId, brightness, contrast,
                        saturation,
                        hue, opacity, zIndex, visible, minResolution, maxResolution);
                    if (layerId >= 0)
                    {
        ...
        public void AddImageLayer(int instanceId, string name, int sourceId, float brightness, float contrast,
            float saturation,
            float hue, float opacity, int zIndex, bool visible, int minResolution, int maxResolution)
        {
            lock (Cave.AppLocks[instanceId])
        ...
        public void AddVectorLayer(int instanceId, string name, int sourceId, float brightness, float contrast,
            float saturation,
            float hue, float opacity, int zIndex, bool visible, int minResolution, int maxResolution, int styleId)
        {
            lock (Cave.AppLocks[instanceId])
            {
                try
                {
                    MapsApp maps = ((MapsApp) Cave.Apps["Maps"].Instances[instanceId]);
                    int layerId = -1;
                    layerId = maps.AddLayer<VectorLayer>(name, (int) LayerTypes.Vector, sourceId, brightness, contrast,
                        saturation,
                        hue, opacity, zIndex, visible, minResolution, maxResolution);
                    if (layerId >= 0)
                    {
        ...
        public void AddTileLayer(int instanceId, string name, int sourceId, float brightness, float contrast,
            float saturation,
        ...
        public void ModifyLayer(int instanceId, int layerId, string name, int sourceId, float brightness, float contrast,
            float saturation,
            float hue, float opacity, int zIndex, bool visible, int minResolution, int maxResolution)
        {
            lock (Cave.AppLocks[instanceId])
            {
                ...
                {
                    MapsApp maps = ((MapsApp) Cave.Apps["Maps"].Instances[instanceId]);
                    maps.ModifyLayer(layerId, name, (int) LayerTypes.Base, sourceId, brightness, contrast, saturation,
                        hue, opacity,
        ...
        public void ModifyHeatmapLayer(int instanceId, int layerId, string name, int sourceId, float brightness,
            ...
            int weight)
        {
            lock (Cave.AppLocks[instanceId])
            {
                ...
                {
                    MapsApp maps = ((MapsApp) Cave.Apps["Maps"].Instances[instanceId]);
                    maps.ModifyLayer(layerId, name, (int) LayerTypes.Heatmap, sourceId, brightness, contrast, saturation,
                        hue, opacity,
                        zIndex, visible, minResolution, maxResolution);
                    maps.Layers.GetValue<HeatmapLayer>(layerId).Modify(gradient, radius, shadow, weight);
                    BroadcastLayer(instanceId, layerId);
        ...
        public void ModifyImageLayer(int instanceId, int layerId, string name, int sourceId, float brightness,
            float contrast, float saturation,
            float hue, float opacity, int zIndex, bool visible, int minResolution, int maxResolution)
        {
            lock (Cave.AppLocks[instanceId])
            {
                ...
                {
                    MapsApp maps = ((MapsApp) Cave.Apps["Maps"].Instances[instanceId]);
                    maps.ModifyLayer(layerId, name, (int) LayerTypes.Image, sourceId, brightness, contrast, saturation,
                        hue, opacity,
                        zIndex, visible, minResolution, maxResolution);
                    BroadcastLayer(instanceId, layerId);
                }
        ...
        public void ModifyVectorLayer(int instanceId, int layerId, string name, int sourceId, float brightness,
            float contrast, float saturation,
            float hue, float opacity, int zIndex, bool visible, int minResolution, int maxResolution, int styleId)
        {
            lock (Cave.AppLocks[instanceId])
            {
                ...
                {
                    MapsApp maps = ((MapsApp) Cave.Apps["Maps"].Instances[instanceId]);
                    maps.ModifyLayer(layerId, name, (int) LayerTypes.Vector, sourceId, brightness, contrast, saturation,
                        hue, opacity,
                        zIndex, visible, minResolution, maxResolution);
                    maps.Layers.GetValue<VectorLayer>(layerId).Modify(maps.Styles.GetValue<Style>(styleId));
                    BroadcastLayer(instanceId, layerId);
        ...
        public void ModifyTileLayer(int instanceId, int layerId, string name, int sourceId, float brightness,
            float contrast, float saturation,
            float hue, float opacity, int zIndex, bool visible, int minResolution, int maxResolution, int preload)
        {
            lock (Cave.AppLocks[instanceId])
            {
                ...
                {
                    MapsApp maps = ((MapsApp) Cave.Apps["Maps"].Instances[instanceId]);
                    maps.ModifyLayer(layerId, name, (int) LayerTypes.Tile, sourceId, brightness, contrast, saturation,
                        hue, opacity,
                        zIndex, visible, minResolution, maxResolution);
                    maps.Layers.GetValue<TileLayer>(layerId).Modify(preload);
                    BroadcastLayer(instanceId, layerId);
        ...
        public void AddView(int instanceId, double[] topLeft, double[] center, double[] bottomRight, float resolution,
        ...
        public void ModifyView(int instanceId, int viewId, double[] topLeft, double[] center, double[] bottomRight,
            float resolution, int zoom, int minResolution, int maxResolution, int minZoom, int maxZoom,
            string projection, float rotation)
        {
            lock (Cave.AppLocks[instanceId])
            {
                try
                {
                    ...
                    View view = maps.Views.GetValue<View>(viewId);
                    Position position = new Position(topLeft, center, bottomRight, resolution, zoom);
                    bool result = maps.ModifyView(viewId, position, minResolution, maxResolution, minZoom, maxZoom,
                        projection, rotation);
                    if (result)
                    {
                        BroadcastView(instanceId, viewId);
        ...
        public void ModifyVectorImageSource(int instanceId, int sourceId, string name, int vectorSourceId, int styleId,
            double ratio)
        {
            lock (Cave.AppLocks[instanceId])
            {
        ...
        public void AddIconStyle(int instanceId, string name, float[] anchor, int anchorOrigin, float[] offset,
            int offsetOrigin, float opacity, float scale,
            bool snapToPixel, bool rotateWithView, float rotation, int width, int height, int imageWidth,
        ...
        public void AddTextStyle(int instanceId, string name, string font, int offsetX, int offsetY, float scale,
        ...
        public void ModifyCircleStyle(int instanceId, int styleId, string name, int fillId, float opacity,
            bool rotateWithView, float rotation, float scale, int radius, bool snapToPixel, int strokeId)
        {
            lock (Cave.AppLocks[instanceId])
            {
                try
        ...
        public void ModifyIconStyle(int instanceId, int styleId, string name, float[] anchor, int anchorOrigin,
            float[] offset, int offsetOrigin, float opacity,
            float scale, bool snapToPixel, bool rotateWithView, float rotation, int width, int height, int imageWidth,
            int imageHeight, string imageSource)
        {
            lock (Cave.AppLocks[instanceId])
            {
        ...
        public void ModifyStrokeStyle(int instanceId, int styleId, string name, string color, int lineCap, int lineJoin,
            int[] lineDash, int miterLimit, int width)
        {
            lock (Cave.AppLocks[instanceId])
            {
                try
        ...
        public void ModifyTextStyle(int instanceId, int styleId, string name, string font, int offsetX, int offsetY,
            float scale, float rotation, string content,
            string textAlign, string textBaseLine, int fillId, int strokeId)
        {
            lock (Cave.AppLocks[instanceId])
            {
        ...
        public void ModifyRegularShapeStyle(int instanceId, int styleId, string name, int fillId, float opacity,
            bool rotateWithView, float rotation, float scale,
            int points, int radius, int radius1, int radius2, int angle, bool snapToPixel, int strokeId)
        {
            lock (Cave.AppLocks[instanceId])
            {
                try
                {
                    ...
                    StrokeStyle stroke = maps.Styles.GetValue<StrokeStyle>(strokeId);
                    maps.Styles.GetValue<RegularShapeStyle>(styleId)
                        .Modify(fill, opacity, rotateWithView, rotation, scale, points, radius, radius1, radius2, angle,
                            snapToPixel, stroke);
                    BroadcastStyle(instanceId, styleId);
                }
                catch (Exception e)
            ...
            bool curve, bool multiCurve, bool multiSurface, string schemaLocation)
        {
            lock (Cave.AppLocks[instanceId])
            {
                try
                {
                    ...
                    if (formatId >= 0)
                    {
                        maps.Formats.GetValue<GMLFormat>(formatId).Modify(gmlVersion, featureNS, featureType, srsName, surface,
                            curve, multiCurve, multiSurface, schemaLocation);
                        BroadcastFormat(instanceId, formatId);
                    }
                }
                catch (Exception e)
        ...
        public void ModifyEsriJSONFormat(int instanceId, int formatId, string name, string geometryName)
        {
            lock (Cave.AppLocks[instanceId])
            {
                try
                {
                    ...
                    maps.Formats.GetValue<EsriJSONFormat>(formatId)
                        .Modify(geometryName);
                    BroadcastFormat(instanceId, formatId);
                }
                catch (Exception e)
        ...
        public void ModifyGeoJSONFormat(int instanceId, int formatId, string name, string geometryName)
        {
            lock (Cave.AppLocks[instanceId])
            {
                try
                {
                    ...
                    maps.Formats.GetValue<GeoJSONFormat>(formatId)
                        .Modify(geometryName);
                    BroadcastFormat(instanceId, formatId);
                }
                catch (Exception e)
        ...
        public void ModifyGMLFormat(int instanceId, int formatId, string name, int gmlVersion, string[] featureNS, string[] featureType,
            string srsName, bool surface, bool curve, bool multiCurve, bool multiSurface, string schemaLocation)
        {
            lock (Cave.AppLocks[instanceId])
            {
                try
                {
                    ...
                    {
                        case 0:
                            maps.ModifyFormat(formatId, name, (int)FormatTypes.GMLBase);
                            break;
                        case 1:
                            maps.ModifyFormat(formatId, name, (int)FormatTypes.GML);
                            break;
                        case 2:
                            maps.ModifyFormat(formatId, name, (int)FormatTypes.GML2);
                            break;
                        case 3:
                            maps.ModifyFormat(formatId, name, (int)FormatTypes.GML3);
                            break;
                        default:
                            maps.ModifyFormat(formatId, name, (int)FormatTypes.GML3);
                            break;
        ...
        public void ModifyKMLFormat(int instanceId, int formatId, string name, bool extractStyles, int[] styleIds)
        {
            lock (Cave.AppLocks[instanceId])
            {
                try
                {
                    ...
                    maps.Formats.GetValue<KMLFormat>(formatId)
                        .Modify(defaultStyle, extractStyles);
                    BroadcastFormat(instanceId, formatId);
                }
                catch (Exception e)
                {
